...
public class CompanyServiceController {
	...
	public ResponseEntity<List<Map<String, Object>>> findAll() {
		List<Map<String, Object>> rows = jdbc.queryForList("SELECT " + SELECT_COLUMNS + " FROM catalogo.company_service ORDER BY created_at DESC");
	...
	public ResponseEntity<Void> delete(@PathVariable Long id) {
		int deleted = jdbc.update("DELETE FROM catalogo.company_service WHERE id = ?", id);
		if (deleted == 0) {
			return ResponseEntity.notFound().build();
		}
