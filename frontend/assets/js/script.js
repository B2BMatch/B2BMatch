...
const WO=[
  {init:'JP',type:'trabajador',name:'Juan Pérez',role:'Desarrollador web freelance · 8 años exp.',tags:['HTML','CSS','JavaScript'],score:'95%'},
  {init:'AL',type:'trabajador',name:'Andrea Lagos',role:'Diseñadora gráfica independiente',tags:['Illustrator','Figma','Canva'],score:'98%'},
  {init:'RM',type:'trabajador',name:'Roberto Muñoz',role:'Gasfíter certificado · Disponible esta semana',tags:['Residencial','Comercial','Urgencias'],score:'91%'},
  {init:'VP',type:'trabajador',name:'Valeria Pinto',role:'Contadora independiente · PYMES',tags:['Contabilidad','IVA','Renta'],score:'93%'},
];
...
function showResults(type){
  ...
  const data=type==='empresa'?CO:type==='trabajador'?WO:[...CO,...WO];
  document.getElementById('r-count').textContent=data.length;
  ['btn-co','btn-wo','btn-all'].forEach(id=>document.getElementById(id).classList.remove('act'));
  if(type==='empresa')document.getElementById('btn-co').classList.add('act');
  else if(type==='trabajador')document.getElementById('btn-wo').classList.add('act');
